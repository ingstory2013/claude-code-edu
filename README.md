...
> ## ⏸️ 현재 자동 발행은 중지돼 있습니다
>
> 두 워크플로의 `schedule` 트리거가 주석 처리돼 있어 정기 실행이 돌지 않습니다.
> 수동 실행(**Actions → 워크플로 선택 → Run workflow**)은 그대로 동작합니다.
>
> **다시 켜려면** — 아래 세 가지를 갖춘 뒤 워크플로 파일의 `schedule` 주석을 풀면 됩니다:
> 1. 시크릿 등록 (`SLACK_BOT_TOKEN`, `SLACK_CHANNEL_ID`, 탐색까지 쓰려면 `ANTHROPIC_API_KEY`)
> 2. 리포 기본 브랜치를 `main`으로 — **스케줄은 기본 브랜치에서만 돕니다**
> 3. 봇이 대상 채널에 초대돼 있을 것
>
> 이미 채널에 발행된 2건은 `state/posted.json`에 기록돼 있어, 재개해도 중복되지 않고
> 3번째 팁부터 이어집니다.
